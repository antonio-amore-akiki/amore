...
/// Initialise the ort runtime from ORT_DYLIB_PATH (load-dynamic feature).
///
/// Must be called once before any `Session::builder()` use. Called at the top of
/// `Reranker::new_with_max_length` so any missing-DLL error surfaces at model-load
/// time rather than as a cryptic linker or runtime fault.
///
/// Idempotent: ort ignores repeated calls after the first commit succeeds.
fn ensure_ort_init() -> Result<()> {
    let dylib_path = std::env::var("ORT_DYLIB_PATH")
        .map_err(|_| anyhow!(
            "ORT_DYLIB_PATH is not set — cannot locate onnxruntime.dll. \
             Set it to the full path of onnxruntime.dll before starting amore. \
             Download from: https://github.com/microsoft/onnxruntime/releases"
        ))?;
    // init_from loads the dylib; commit() returns bool (false = already committed, which is fine)
    ort::init_from(&dylib_path)
        .with_context(|| format!("ort: failed to load ONNX Runtime dylib from {dylib_path}"))?
        .commit();
    Ok(())
}
