...
fn enabled() -> bool {
    std::env::var("AMORE_TEST_E2E").ok().as_deref() == Some("1")
}
...
fn unique_collection(suffix: &str) -> String {
    ...
    let nanos = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    format!("amore_e2e_{suffix}_{nanos:x}")
}

#[tokio::test]
#[ignore = "requires AMORE_TEST_E2E=1 + Qdrant + Ollama daemons"]
async fn end_to_end_index_and_recall() {
    if !enabled() {
        eprintln!("AMORE_TEST_E2E not set to 1 — skipping");
        return;
    }
