...
//
// Gated by AMORE_TEST_OLLAMA=1; uses ensure_daemons.{ps1,sh} before invoke.
// Run with:
//     $env:AMORE_TEST_OLLAMA=1
//     cargo test -p amore-core --test ensemble_real_ollama -- --ignored --nocapture
//
...
use amore_core::ensemble::{AgentRole, LlmAgent, Orchestrator};
...
#[tokio::test]
#[ignore = "requires AMORE_TEST_OLLAMA=1 + live Ollama on 11434 with qwen3:8b"]
async fn architect_plus_skeptic_decide_against_real_qwen3() {
    if !enabled() {
        eprintln!("AMORE_TEST_OLLAMA not set; skipping");
        return;
    }

    let base =
        std::env::var("AMORE_OLLAMA_URL").unwrap_or_else(|_| "http://127.0.0.1:11434".to_string());
