...
fn build_index() -> TantivyIndex {
    let mut idx = TantivyIndex::new(std::path::Path::new(":memory:"))
        .expect("create in-memory TantivyIndex");
...
fn tantivy_rank_order_matches_fts5_baseline() {
    ...
    assert_eq!(passed, total, "not all queries passed rank-order parity check");
}
