...
//
// Gated by AMORE_TEST_MINING=1. Run with:
//     $env:AMORE_TEST_MINING=1
//     cargo test -p amore-core --test mining_real_ledger -- --ignored --nocapture
//
...
use amore_core::mining::{mine_from_jsonl, parse_ledger};

fn enabled() -> bool {
    std::env::var("AMORE_TEST_MINING").ok().as_deref() == Some("1")
}

fn ledger_path() -> std::path::PathBuf {
    if let Ok(p) = std::env::var("AMORE_TEST_MINING_LEDGER") {
...
#[test]
#[ignore = "requires AMORE_TEST_MINING=1 + ~/.claude/state/edit-rationale-ledger.jsonl"]
fn mining_processes_real_ledger_without_panic() {
    if !enabled() {
        eprintln!("AMORE_TEST_MINING not set; skipping");
        return;
    }
...
#[test]
#[ignore = "requires AMORE_TEST_MINING=1"]
