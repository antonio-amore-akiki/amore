...
/// timeout-cap path: when delay_ms > AMORE_TIMEOUT_MS, the with_timeout
...
async fn b3_slow_embedder_times_out_and_flips_ollama_unavailable() {
    ...
    // hang the recall — the AMORE_TIMEOUT_MS cap fires, embed returns Err,
    // BM25 lane serves the response with ollama_unavailable=true.
    unsafe { std::env::set_var("AMORE_TIMEOUT_MS", "150") };
    ...
    unsafe { std::env::remove_var("AMORE_TIMEOUT_MS") };
