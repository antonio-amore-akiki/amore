...
// ---------------------------------------------------------------------------
#[derive(Debug, Clone)]
struct VecHit { id: String, score: f32 }
#[derive(Debug, Clone)]
struct Bm25Hit { id: String, score: f32 }

#[derive(Debug, Clone, serde::Serialize, serde::Deserialize, PartialEq)]
struct FusedHit { id: String, rrf_score: f32, rank: usize }
...
fn rrf_fuse(vec_hits: &[VecHit], bm_hits: &[Bm25Hit], top_k: usize) -> Vec<FusedHit> {
    ...
    let mut fused: Vec<FusedHit> = acc
        .into_iter()
        .map(|(id, rrf_score)| FusedHit { id, rrf_score, rank: 0 })
        .collect();
    fused.sort_by(|a, b| {
    ...
    for (i, h) in fused.iter_mut().enumerate() { h.rank = i + 1; }
    fused
}
...
fn vec_hit(id: &str, rank: usize) -> VecHit {
    VecHit { id: id.to_string(), score: 1.0 - 0.01 * rank as f32 }
}
fn bm25_hit(id: &str, rank: usize) -> Bm25Hit {
    Bm25Hit { id: id.to_string(), score: 10.0 - 0.3 * rank as f32 }
}
...
fn input_pairs() -> Vec<(&'static str, Vec<&'static str>, Vec<&'static str>)> {
    vec![
        (
            "pair-1",
            vec!["d-005","d-011","d-017","d-023","d-029","d-000","d-006","d-012","d-018","d-024"],
            vec!["d-011","d-005","d-029","d-017","d-023","d-007","d-013","d-019","d-025","d-001"],
        ),
        (
            "pair-2",
            vec!["d-000","d-006","d-012","d-018","d-024","d-001","d-007","d-013","d-019","d-025"],
            vec!["d-008","d-014","d-020","d-026","d-002","d-000","d-006","d-003","d-009","d-015"],
        ),
        (
            "pair-3",
            vec!["d-000","d-001","d-002","d-003","d-004","d-005","d-006","d-007","d-008","d-009",
                 "d-010","d-011","d-012","d-013","d-014","d-015","d-016","d-017","d-018","d-019"],
            vec!["d-019","d-018","d-017","d-016","d-015","d-014","d-013","d-012","d-011","d-010",
                 "d-009","d-008","d-007","d-006","d-005","d-004","d-003","d-002","d-001","d-000"],
        ),
        ("pair-4", vec!["d-022","d-028","d-016","d-010","d-004"], vec![]),
        ("pair-5", vec![], vec!["d-003","d-009","d-015","d-021","d-027"]),
    ]
}
...
struct PairResult { pair_id: String, top_k: usize, fused: Vec<FusedHit> }

#[derive(Debug, serde::Serialize, serde::Deserialize)]
struct RrfFixture { schema_version: u32, rrf_k: f32, pairs: Vec<PairResult> }

fn fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests").join("fixtures").join("rrf_baseline.json")
}
...
fn rrf_fusion_baseline() {
    ...
    let pair_results: Vec<PairResult> = input_pairs()
        ...
        .map(|(pair_id, vec_ids, bm25_ids)| {
            let vh: Vec<VecHit> = vec_ids.iter().enumerate().map(|(r, id)| vec_hit(id, r)).collect();
            let bh: Vec<Bm25Hit> = bm25_ids.iter().enumerate().map(|(r, id)| bm25_hit(id, r)).collect();
            PairResult { pair_id: pair_id.to_string(), top_k: TOP_K, fused: rrf_fuse(&vh, &bh, TOP_K) }
        })
        .collect();
    ...
    if rebase || !fixture_path.exists() {
        let fixture = RrfFixture { schema_version: 1, rrf_k: RRF_K, pairs: pair_results };
    ...
    let raw = std::fs::read_to_string(&fixture_path)
        .unwrap_or_else(|_| panic!("RRF fixture missing at {}; run with AMORE_BM25_REBASE=1", fixture_path.display()));
    let fixture: RrfFixture = serde_json::from_str(&raw).expect("parse RRF fixture");

    assert_eq!(fixture.rrf_k, RRF_K, "RRF k constant changed: fixture={} impl={}", fixture.rrf_k, RRF_K);
    assert_eq!(fixture.pairs.len(), pair_results.len(), "pair count mismatch");

    for (expected, actual) in fixture.pairs.iter().zip(pair_results.iter()) {
        assert_eq!(expected.pair_id, actual.pair_id);
        assert_eq!(expected.fused.len(), actual.fused.len(),
            "fused hit count mismatch for pair {}", expected.pair_id);
        for (e, a) in expected.fused.iter().zip(actual.fused.iter()) {
            assert_eq!(e.id, a.id,
                "rank {} id mismatch for pair {}: expected {} got {}", e.rank, expected.pair_id, e.id, a.id);
            assert_eq!(e.rank, a.rank,
                "rank mismatch for doc {} in pair {}", e.id, expected.pair_id);
            assert!((e.rrf_score - a.rrf_score).abs() < 1e-6,
                "RRF score drift for doc {} in pair {}: expected {:.8} got {:.8}",
                e.id, expected.pair_id, e.rrf_score, a.rrf_score);
        }
    }
    println!("RRF baseline: {} pairs verified against {}", pair_results.len(), fixture_path.display());
}
...
fn rrf_formula_exact_k60() {
    ...
    assert!((a.rrf_score - 1.0_f32 / 60.0).abs() < 1e-6, "A: {}", a.rrf_score);
    assert!((b.rrf_score - 1.0_f32 / 61.0).abs() < 1e-6, "B: {}", b.rrf_score);
}
