...
// scenario this models: a single amore-mcp instance multiplexing N IDE
// clients (or two amore processes on the same machine — a CLI + an MCP
...
fn fresh_tmp_db_path() -> PathBuf {
    ...
    let nanos = std::time::SystemTime::now()
        ...
        .map(|d| d.as_nanos())
        .unwrap_or(0);
    std::env::temp_dir().join(format!("amore_b6_{nanos:x}_{n}.db"))
}
