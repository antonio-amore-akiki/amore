...
async fn ensure_qdrant_collection(client: &Qdrant) -> Result<()> {
    ...
    eprintln!("[seed] created collection '{COLLECTION_NAME}' (dim={VECTOR_DIM}, Cosine, int8 quant)");
    Ok(())
}
...
async fn main() -> Result<()> {
    ...
    let sqlite_path = args
        .sqlite_path
        .clone()
        .unwrap_or_else(sqlite_path_default);
    ...
    while idx < total {
            ...
            points.push(PointStruct {
                id: Some(PointId {
                    point_id_options: Some(
                        qdrant_client::qdrant::point_id::PointIdOptions::Num(i),
                    ),
                }),
                payload,
        ...
        if idx % 10_000 == 0 || idx == total {
            eprintln!("[seed] progress: {idx}/{total} ({:.1}%)", idx as f64 / total as f64 * 100.0);
        }
    }

    eprintln!("[seed] done: {total} observations seeded to Qdrant collection '{COLLECTION_NAME}' + SQLite {}", sqlite_path.display());
    Ok(())
}
