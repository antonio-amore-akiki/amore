...
// trait normalizes the contract so the CLI dispatch (`obelion init <ide>`) is
// uniform.
//
...
/// Helper: merge an `obelion` entry into the `mcpServers` object inside a JSON
/// config (Claude Code and Cursor share this schema). Returns the pretty-
/// printed JSON ending with a trailing newline.
///
/// Idempotent: if `mcpServers.obelion` already equals `entry`, the input is
/// returned unchanged. The rest of the existing config (including unrelated
...
mod tests {
    ...
    fn fresh_tmp_dir() -> PathBuf {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap_or_default()
            .as_nanos();
        let p = std::env::temp_dir().join(format!("obelion-adapter-test-{nanos:x}-{n}"));
    ...
    impl IdeAdapter for InlineAdapter {
        ...
        fn merge_into(&self, existing: &str) -> Result<String> {
            merge_mcp_servers(
                existing,
                "obelion",
                serde_json::json!({"command":"obelion-mcp","args":[]}),
            )
        }
    }
    ...
    fn merge_into_empty_starts_object() {
        let m = merge_mcp_servers("", "obelion", serde_json::json!({"command":"x"})).unwrap();
        assert!(m.contains("\"obelion\""));
        assert!(m.contains("\"command\""));
    }

    #[test]
    fn merge_into_existing_obelion_is_noop() {
        ...
        .unwrap();
        assert_eq!(existing, m, "no diff when obelion already matches");
    }
}
