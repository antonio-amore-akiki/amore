...
mod tests {
    ...
    fn upsert_then_get_project_roundtrip() {
        let wm = wm();
        wm.upsert_project("obelion", json!({"lang":"rust"}))
            .unwrap();
        let p = wm.get_project("obelion").unwrap().unwrap();
        assert_eq!(p.name, "obelion");
    ...
    fn upsert_overwrites_existing() {
        let wm = wm();
        wm.upsert_project("obelion", json!({"v":1})).unwrap();
        wm.upsert_project("obelion", json!({"v":2})).unwrap();
        assert_eq!(wm.get_project("obelion").unwrap().unwrap().payload["v"], 2);
    }
