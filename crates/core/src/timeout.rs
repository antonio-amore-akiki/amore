...
// rmcp request, etc.) is capped at OBELION_TIMEOUT_MS milliseconds (default
...
/// Resolve the active timeout from `OBELION_TIMEOUT_MS` env.
pub fn resolve_timeout() -> Duration {
    parse_timeout(std::env::var("OBELION_TIMEOUT_MS").ok().as_deref())
}
