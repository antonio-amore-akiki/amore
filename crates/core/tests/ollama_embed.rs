...
//! without OBELION_TEST_OLLAMA=1 set — preventing surprise network calls.
...
#[tokio::test]
#[ignore = "requires OBELION_TEST_OLLAMA=1 + Ollama daemon + nomic-embed-text model"]
async fn embed_returns_expected_dimension_vector_for_short_prompt() {
    if !enabled() {
        eprintln!("OBELION_TEST_OLLAMA not set to 1 — skipping");
        return;
    }
...
#[tokio::test]
#[ignore = "requires OBELION_TEST_OLLAMA=1 + Ollama daemon"]
