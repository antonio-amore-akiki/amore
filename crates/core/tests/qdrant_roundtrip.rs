...
fn unique_collection(suffix: &str) -> String {
    ...
    let nanos = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    format!("obelion_test_{suffix}_{nanos:x}")
}

#[tokio::test]
#[ignore = "requires OBELION_TEST_QDRANT=1 + Qdrant + Ollama daemons"]
async fn upsert_then_search_returns_top_match() {
    if !enabled() {
        eprintln!("OBELION_TEST_QDRANT not set to 1 — skipping");
        return;
    }
...
#[tokio::test]
#[ignore = "requires OBELION_TEST_QDRANT=1 + Qdrant daemon"]
