...
//! `obelion` MCP server. Implements [`obelion_core::ide_adapter::IdeAdapter`].
//!
...
impl IdeAdapter for CodexAdapter {
    ...
    fn merge_into(&self, existing: &str) -> Result<String> {
        merge_codex_toml(existing, "obelion", &self.command)
    }
}
...
mod tests {
    ...
    fn merge_into_empty_creates_table() {
        ...
        assert!(merged.contains("command = \"obelion-mcp\""));
