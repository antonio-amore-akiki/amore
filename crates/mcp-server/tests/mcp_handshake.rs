//! Integration test: spawn `obelion-mcp` binary, perform JSON-RPC handshake
...
//!
//! Prerequisites:
//!   - Qdrant daemon at OBELION_QDRANT_URL (default http://127.0.0.1:6334 gRPC)
//!   - Ollama daemon at OBELION_OLLAMA_URL (default http://127.0.0.1:11434)
//!
...
fn enabled() -> bool {
    std::env::var("OBELION_TEST_MCP").ok().as_deref() == Some("1")
}

#[tokio::test]
#[ignore = "requires OBELION_TEST_MCP=1 + live Qdrant + Ollama"]
async fn handshake_lists_recall_tool() {
    if !enabled() {
        eprintln!("OBELION_TEST_MCP not set; skipping");
        return;
    }
    ...
    let bin = env!("CARGO_BIN_EXE_obelion-mcp");
    let mut child = Command::new(bin)
        .env("OBELION_COLLECTION", &unique_collection)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
    ...
    if let Ok(qs) =
        obelion_core::qdrant_store::QdrantStore::open(&qdrant_url, &unique_collection).await
    {
