// obelion-eval — token-reduction harness (A9 / S13 gate).
//
// Measures the central product claim: obelion's canonical-docs router reduces
...
#[derive(Parser)]
#[clap(
    about = "Measures token reduction of obelion canonical-docs router vs raw-context baseline."
)]
struct Args {
...
fn main() -> Result<()> {
    ...
    let summary_row = format!(
        "{ts}\tA9\tcargo run -p obelion-eval --bin token-reduction\t{verdict}\ttoken_reduction\tavg={avg:.1}%/worst={worst:.1}%(class={worst_class},id={worst_id})/N={n}\tcrates/eval/src/main.rs\tUNCOMMITTED"
    );
    let per_fixture: Vec<String> = results
        .iter()
        .map(|r| {
            format!(
                "{ts}\tA9-fx-{id}\tcargo run -p obelion-eval --bin token-reduction\t{v}\t{cls}_reduction\tbaseline={b}/optimized={o}/reduction={p:.1}%/hits={h}\tcrates/eval/fixtures/{id}.json\tUNCOMMITTED",
                id = r.id,
