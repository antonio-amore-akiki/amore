// QA A3 — `amore init <ide>` real-disk matrix for all 7 adapters.
//
...
//      backup file holds pre-edit bytes; new file contains both old + amore
...
fn fresh_tmp_dir() -> PathBuf {
    ...
    let nanos = std::time::SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    let d = std::env::temp_dir().join(format!("amore-a3-{nanos:x}-{n}"));
...
fn verify_adapter<A: IdeAdapter>(adapter: &A, target_filename: &str) -> String {
    ...
    // Tamper: write a baseline that parses correctly per the schema but does
    // NOT contain the amore entry. The next apply MUST detect the drift and
    // produce Updated{path, backup}.
    ...
    assert!(
        merged.contains("amore"),
        "{target_filename}: post-merge content contains amore entry"
    );
    assert!(
