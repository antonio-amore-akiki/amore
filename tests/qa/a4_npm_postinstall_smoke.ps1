...
# downloads amore-v<VER>-x86_64-pc-windows-msvc.zip from the GitHub Release,
# extracts to bin/amore.exe + bin/amore-mcp.exe, and `--version` reports
...
#
# Auth note: while the amore repo is private, postinstall.js consumes
# GITHUB_TOKEN/GH_TOKEN/AMORE_GITHUB_TOKEN as Bearer credentials. This
...
$sandbox = Join-Path $env:TEMP "amore-a4-$([guid]::NewGuid().ToString('N').Substring(0,8))"
...
if (-not $ghToken) {
    Write-Error "[a4] no GH token available via 'gh auth token'. Amore repo is private during MVP — install would 404."
    exit 4
}
...
$installedPkg = Join-Path $sandbox "node_modules\@anto\amore"
$binDir = Join-Path $installedPkg "bin"
$amoreExe = Join-Path $binDir "amore.exe"
$amoreMcpExe = Join-Path $binDir "amore-mcp.exe"

foreach ($f in @($amoreExe, $amoreMcpExe)) {
    if (-not (Test-Path $f)) {
...
# --- step 6: run --version on the amore binary and assert tag match
Write-Output "[a4] running: amore.exe --version"
$verOut = (& $amoreExe --version 2>&1 | Out-String).Trim()
Write-Output "[a4] amore --version -> $verOut"

if ($verOut -notmatch [regex]::Escape($expectedVersion)) {
    Write-Error "[a4] amore --version output did not contain ${expectedVersion}: '$verOut'"
    exit 8
}

# --- cleanup of pack.tgz + sandbox (sandbox optional; keep on demand)
Remove-Item -Path $packPath -Force -ErrorAction SilentlyContinue
if ($env:AMORE_A4_KEEP_SANDBOX -ne "1") {
    Remove-Item -Path $sandbox -Recurse -Force -ErrorAction SilentlyContinue
    Write-Output "[a4] sandbox cleaned"
} else {
    Write-Output "[a4] sandbox kept at $sandbox (AMORE_A4_KEEP_SANDBOX=1)"
}
