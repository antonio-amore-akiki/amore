...
## Google SRE Methodology — SLI/SLO Definitions

Per Google SRE Ch.4: each SLO requires SLI definition, measurement protocol,
aggregation interval, and validity conditions.

### Service Class A — user-facing recall

**SLO targets (rolling 30-day window)**:
- Availability: 99.9% (≤ 43.2 min downtime/month)
- Latency p99: ≤ 500 ms @ ≤ 10K corpus; ≤ 2000 ms @ ≤ 100K corpus
- Throughput: ≥ 50 QPS sustained

**SLI measurement**:
- Prometheus histograms `amore_db_operation_duration_seconds` (latency)
- Prometheus counters `amore_db_operation_total{status="ok"}` and
  `amore_db_operation_total{status="error"}` (availability + error rate)
- Monitoring: NEUTRAL-monitored (Prometheus scrape, not self-reported)

**Aggregation**: 30-day rolling window; re-evaluated weekly.

**Validity conditions**: corpus size ≤ 1M docs. Above 1M docs is Service Class B
scope (out of v1.0 SLO). Measurements invalid during scheduled maintenance
windows (documented in release notes).

### Service Class B — storage (sled L2 + Qdrant)

**SLO targets**:
- Durability: 99.99% (no data loss except unaddressable hardware failure)
- WAL fsync-write success rate: ≥ 99.9% per write call

**SLI measurement**: write-path instrumentation; confirmed via WAL replay
integrity check on startup. Hardware failure events logged and excluded
from durability budget via explicit incident postmortem.

### Service Class C — gRPC health endpoint

**SLO targets**:
- Availability: 99.95% (rolling 30d)

**SLI measurement**: synthetic probe every 30s;
`amore_grpc_health_check_total{status="ok|error"}`.

---

Source: sre.google/sre-book/service-level-objectives; sre.google/sre-book/embracing-risk
