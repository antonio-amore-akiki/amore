# Amore Quickstart — Cursor
stable: true
topic: amore quickstart cursor mcp setup
tier: 1 (non-technical user)
last_verified: 2026-05-26 (amore v0.3.1-live-fire)

Connect Amore's long-term memory to Cursor in under five minutes. No coding required.
...
**What it writes:** `~/.cursor/mcp.json` — Cursor's global MCP server registry.
Amore merges one entry into the top-level `mcpServers` map:
...
Existing entries (other MCP servers) are preserved.
Running the command a second time is safe — it is idempotent.

**Note:** A `--workspace <path>` flag for project-scoped registration is
planned for a future sprint (S9). The current default registers globally.

**Restart Cursor** after running `amore init cursor`. Cursor caches its MCP
...
Paste this prompt in a new Cursor chat:
...
2. Open Cursor's output panel and search for `amore-mcp`. A line like
...
**"Cursor doesn't see Amore after `amore init cursor`"**
Restart Cursor. It caches the MCP server list at startup; a full restart is
required to pick up changes to `~/.cursor/mcp.json`.
