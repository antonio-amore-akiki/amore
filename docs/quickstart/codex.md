# Amore Quickstart — Codex CLI
stable: true
topic: amore quickstart codex mcp setup
tier: 1 (non-technical user)
last_verified: 2026-05-26 (amore v0.3.1-live-fire)

Connect Amore's long-term memory to the OpenAI Codex CLI in under five minutes.
...
**What it writes:** `~/.codex/config.toml` — Codex CLI's configuration file.
Amore merges one block into the `[mcp_servers]` table:
...
If the environment variable `CODEX_HOME` is set, Amore writes to
`$CODEX_HOME/config.toml` instead of `~/.codex/config.toml`.

Existing tables (`[model]`, other `[mcp_servers.*]` entries, etc.) are
preserved. Running the command a second time is safe — it is idempotent.

**Restart Codex CLI** after running `amore init codex`.

---

## 3. Verify the handshake

Run Codex CLI and submit this prompt:
...
2. Check Codex CLI's debug output for an `amore-mcp` line showing it started.
...
**"Codex CLI doesn't see Amore after `amore init codex`"**
Restart Codex CLI. It caches the MCP server list at startup; a full restart
is required to pick up changes to `~/.codex/config.toml`.
