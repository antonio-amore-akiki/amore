# Amore Quickstart — Windsurf
stable: true
topic: amore quickstart windsurf mcp setup
tier: 1 (non-technical user)
last_verified: 2026-05-26 (amore v0.3.1-live-fire)

Connect Amore's long-term memory to Windsurf (by Codeium) in under five minutes.
...
**What it writes:** `~/.codeium/windsurf/mcp_config.json` — Windsurf uses a
flat per-user `~/.codeium/` directory on every OS (no platform variation).
...
**Restart Windsurf** after running `amore init windsurf`.

---

## 3. Verify the handshake

Open a Windsurf chat and submit this prompt:
...
2. Open Windsurf's output panel and search for `amore-mcp`. A line confirming
   it started means the server loaded successfully.
...
**"Windsurf doesn't see Amore after `amore init windsurf`"**
Restart Windsurf. It caches the MCP server list at startup; a full restart is
required to pick up changes to `~/.codeium/windsurf/mcp_config.json`.
